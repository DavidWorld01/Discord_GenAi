    ...
    if author == bot.user:
        ...
        if user_id in bot.recent_user_messages and time.time() - bot.recent_user_messages[user_id] < 16:
            ...
            if len(ctx.message.content)<5 or ctx.message.content[4]=='@':
                ...
                link_url_a = 'https://t.me/miraAIcoin'
                link_text_a = 'Join miraAI'
                link_a = f'[{link_text_a}]({link_url_a})'
                link_url_b = 'https://miraai.io/'
                link_text_b = 'Website'
                link_b = f'[{link_text_b}]({link_url_b})'
                link_url_c = 'https://miraai.io/'
                link_text_c = 'Buy'
                link_c = f'[{link_text_c}]({link_url_c})'
                aa= f"Image generated by {author.mention} : {caption}\n> {link_a} | {link_b} | {link_c} <"
